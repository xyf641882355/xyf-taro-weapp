...
function shallowEqual(obj1, obj2) {
  ...
  for (var i = 0; i < obj1Keys.length; i++) {
    ...
    if (
      ...
      !objectIs(obj1[obj1KeyItem], obj2[obj1KeyItem])
    ) {
      return false;
    }
  }
...
var nextTick = function nextTick(fn) {
  ...
  fn =
    typeof fn === 'function'
      ? (_fn = fn).bind.apply(_fn, [null].concat(args))
      : fn;
  ...
  if (process.env.NODE_ENV !== 'production') {
    (function () {
...

      {
        ...
        lowPriorityWarningWithoutStack = function lowPriorityWarningWithoutStack(
          condition,
          format
        ) {
          if (format === undefined) {
          ...
          if (
            ...
            shouldDiffObject &&
              Object.keys(fromItem).some(function (key) {
                if (
                  typeof toItem[key] === 'undefined' &&
                  typeof fromItem[key] !== 'undefined'
                ) {
          ...
          if (!this.map[compid]) {
            Object.defineProperty(this.map, compid, {
              ...
              set: function set(props) {
                ...
                var component =
                  observers[compid] && observers[compid].component;
                var ComponentClass =
                  observers[compid] && observers[compid].ComponentClass;
                if (!component || !ComponentClass || !component.__isReady)
                  return;
                var extraProps =
                  (component.$scope &&
                    component.$scope.data &&
                    component.$scope.data.extraProps) ||
                  null;
                var nextProps = filterProps(
...
function bindStaticFns(weappComponentConf, ComponentClass) {
  for (var key in ComponentClass) {
    typeof ComponentClass[key] === 'function' &&
      (weappComponentConf[key] = ComponentClass[key]);
  } // 低版本 IOS 下部分属性不能直接访问
...
function processEvent(eventHandlerName, obj) {
  ...
  obj[eventHandlerName] = function (event) {
    ...
    if (
      ...
      event.detail.__arguments.length > 0
    ) {
    ...
    if (!isAnonymousFn) {
      ...
      realArgs = _toConsumableArray(datasetArgs).concat(
        _toConsumableArray(detailArgs),
        [event]
      );
    } else {
      // 匿名函数，会将scope作为第一个参数
      ...
      realArgs = [_scope].concat(
        _toConsumableArray(datasetArgs),
        _toConsumableArray(detailArgs),
        [event]
      );
    }
...
function filterParams(data) {
  ...
  for (var paramName in defaultParams) {
    res[paramName] =
      paramName in data ? data[paramName] : defaultParams[paramName];
  }
...
function componentTrigger(component, key, args) {
  ...
  component[key] &&
    ...
    (_component$key = component[key]).call.apply(
      _component$key,
      [component].concat(_toConsumableArray(args))
    );
...
function createComponent(ComponentClass, isPage) {
  ...
  componentInstance._constructor &&
    componentInstance._constructor(componentProps);

  try {
    Current.current = componentInstance;
    Current.index = 0;
    componentInstance.state =
      componentInstance._createData() || componentInstance.state;
  } catch (err) {
    if (isPage) {
      console.warn(
      ...
      if (isPage) {
        ...
        if (hasParamsCache) {
          params = Object.assign(
            {},
            ComponentClass.defaultParams,
            cacheDataGet(this.data[routerParamsPrivateKey], true)
          );
        } else {
          // 直接启动，非内部跳转
        ...
        if (cacheDataHas(this.data[preloadPrivateKey])) {
          this.$component.$preloadData = cacheDataGet(
            this.data[preloadPrivateKey],
            true
          );
        } else {
  ...
  if (isPage) {
    ...
    pageExtraFns.forEach(function (fn) {
      if (
        ...
        typeof componentInstance[fn] === 'function'
      ) {
        weappComponentConf.methods[fn] = function () {
          ...
          if (component[fn] && typeof component[fn] === 'function') {
            ...
            // eslint-disable-next-line no-useless-call
            return (_component$fn = component[fn]).call.apply(
              _component$fn,
  ...
  ComponentClass['$$events'] &&
    bindEvents(weappComponentConf, ComponentClass['$$events'], isPage);

  if (
    ComponentClass['externalClasses'] &&
    ComponentClass['externalClasses'].length
  ) {
...
function hasNewLifecycle(component) {
  ...
  return (
    isFunction(getDerivedStateFromProps) || isFunction(getSnapshotBeforeUpdate)
  );
}
...
function callGetDerivedStateFromProps(component, props, state) {
  ...
  if (isFunction(getDerivedStateFromProps)) {
    ...
    if (!isUndefined(partialState)) {
      ...
      console.warn(
        'getDerivedStateFromProps 没有返回任何内容，这个生命周期必须返回 null 或一个新对象。'
      );
    }
  }
...
function updateComponent(component) {
  ...
  if (isDEV && __propTypes) {
    ...
    if (isUndefined(componentName)) {
      var names = component.constructor
        .toString()
        .match(/^function\s*([^\s(]+)/);
  ...
  var stateFromProps = callGetDerivedStateFromProps(
    component,
    nextProps,
    state
  );
...
function mountComponent(component) {
  ...
  var newState = callGetDerivedStateFromProps(
    component,
    props,
    component.state
  );
  ...
  if (component.$usedState && component.$usedState.length) {
    ...
    component.$usedState.forEach(function (key) {
      ...
      if (_typeof(val) === 'object') {
        if (isEmptyObject(val)) return internal_safe_set(_data, key, {}); // 避免筛选完 Fn 后产生了空对象还去渲染
...
var BaseComponent =
  ...
  (function () {
    ...
    function BaseComponent() {
      ...
      this.isTaroComponent =
        this.$componentType && this.$router && this._pendingStates;
    }
  ...
  (function (_Component) {
    ...
    function PureComponent() {
      ...
      _this = _possibleConstructorReturn(
        this,
        (_getPrototypeOf2 = _getPrototypeOf(PureComponent)).call.apply(
          _getPrototypeOf2,
          [this].concat(args)
        )
      );
    ...
    if (!onAndSyncApis[key] && !noPromiseApis[key]) {
      taro[key] = function (options) {
        ...
        if (
          ...
          if (Component) {
            ...
            if (component.componentWillPreload) {
              ...
              var urlQueryStr = hasMark
                ? obj.url.substring(MarkIndex + 1, obj.url.length)
                : '';
              var params = queryToJson(urlQueryStr);
              obj.url +=
                (hasMark ? '&' : '?') +
                ''.concat(preloadPrivateKey, '=').concat(cacheKey);
        ...
        if (useDataCacheApis[key]) {
          ...
          var _urlQueryStr = _hasMark
            ? _url.substring(_MarkIndex + 1, _url.length)
            : '';
          ...
          obj.url +=
            (_hasMark ? '&' : '?') +
            ''.concat(routerParamsPrivateKey, '=').concat(_cacheKey);
